...
class RegisterViewModel  @Inject constructor(private  val userRepository: UserRepository) : ViewModel(){
    ...
    var registerUserLiveData = userRepository.userLiveData

    fun registerUser(request: RegisterRequest) = viewModelScope.launch(Dispatchers.IO) {
        userRepository.registerUser(request)
        registerUserLiveData = userRepository.userLiveData
    }
}